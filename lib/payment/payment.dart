...
class PaymentsTest extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ConsCubitIntro,cons_StatesIntro>(
      builder:(ctx,state){
        return Scaffold(
          appBar: AppBar(
           ...
           child: SizedBox(
                ...
                child: InAppWebView(
                  ...
                  onLoadStop: (InAppWebViewController controller, url)async{
                    await controller.evaluateJavascript(source: "document.documentElement.innerText").then((value) {
                      if(value.toString().contains("Transaction not completed")){
                        scrollController.jumpTo(scrollController.position.minScrollExtent);
                        myToast(message: "Transaction not completed");
                        navigateToFinish(context, "PaymentError(cubit)");
                      }else if (value.toString().contains("Transaction successful")){
                        scrollController.jumpTo(scrollController.position.minScrollExtent);
