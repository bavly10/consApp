...
class ConsCubitIntro extends Cubit<cons_StatesIntro> {
  ...
  Future<void> getSpecIntro(id) async {
    ...
    final url = Uri.parse(
        "$base_api/users?_start=$limit&_limit=2&_where[specailst]=$id&_where[Confirmed]=true");
    ...
    if (res.statusCode == 200) {
      if (list.isEmpty) {
        ...
        for (var value in list) {
          final pro =
              specIntro.indexWhere((element) => element.id == value['id']);
          if (pro >= 0) {
            specIntro[pro] = UserStrapi(
              introPrice: value["introPrice"],
              id: value["id"],
              ...
              posts: (value['posts'] as List<dynamic>)
                  ...
                  .toList(),
              filesIntros: (value['files_intros'] as List<dynamic>)
                  .map((e) => FilesIntro(
            ...
            specIntro.add(UserStrapi(
              ...
              posts: (value['posts'] as List<dynamic>)
                  .map((e) => Post_user(
                      content: e["content"],
                      publishedAt: e["published_at"],
                      imgPost: e['img_post'] == null
                          ? null
                          : Img_user.fromJson(e['img_post'])))
                  .toList(),
              filesIntros: (value['filesusers'] as List<dynamic>)
                  .map((e) => FilesIntro(
